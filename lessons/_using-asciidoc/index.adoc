...
[IMPORTANT]
====
Due to the nature of AsciiDoc, there are several CSEC-specific modifications/additions to the basic syntax. Of note:

. Write C++ as `{cpp}`.
  This is required as two consecutive pluses (&plus;&plus;) is part of AsciiDoc syntax.
  For replacing just that, use `{pp}`.
. If you wish for code blocks to have line numbers, __do not__ use the `linenums` attribute.
  Instead, add the "linenos" role to the block, i.e. `[role="linenos"]`.
====
...
int main() {
    ...
    printf("Note that you don't need to specify a language; this is because the default is set to C++.");

    return 0;
}
----

.This is also {cpp}, but explicitly stated with line numbers, a title, and a callout.
[source,cpp,role="linenos"]
----
void main() {
    // hello! <1>
}
